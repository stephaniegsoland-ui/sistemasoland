...
async def compare_vehicle_inspection(
    ...
    # If the changed area is unexpectedly large, try a more conservative threshold to split regions
    ...
    draw = PIL_ImageDraw.Draw(output_image)
    ...
    for idx, region in enumerate(regions, start=1):
        ...
        # prepare a color crop for classifier (use area from after_img)
        color_crop = after_img.crop((x0, y0, x1, y1)).convert("RGB")
        bio = BytesIO()
        color_crop.save(bio, format="PNG")
        bio_bytes = bio.getvalue()

        # filter out background regions with custom CLIP vehicle/background labels
        vehicle_check_labels = [
            "vehicle",
            "car",
            "truck",
        ...
        # Heurística mejorada para clasificar el tipo de diferencia
